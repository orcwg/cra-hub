...
<table>
  ...
  <tbody>
    ...
    <tr>
      ...
      <td><!-- Comments -->Suggest to amend definitions to align with the industry accepted definitions (such as OCI, for example), instead of establishing the new requirements (accounting for changes in technology). Suggest to amend The technical details of the container runtime definition to make it compatible with the industry accepted definitions. For example, isolation is not a prerequisite for a running container. Suggest removing "This category includes but is not limited to low-level container runtimes and high-level container runtimes." This is an unnecessary enhancement to distinguish between more user-friendly interfaces of some container runtimes (like containerd) than their counterpart upon which they run (like runc or crun).</td>
      <td><!-- Proposed change -->[...] Container runtime systems are software products with digital elements that manage the lifecycle of containers running on a host operating system, allocating resources <span style="color:red"> <del> and providing isolation between each container and the rest of the system, </del> </span> through operating system level or application-level <span style="color:red"> <del> virtualisation. </del> </span> In the context of this category of products, a container is <span style="color:blue"> a process that runs a container image that has established limited namespace and mount points </span> <span style="color:red"> <del> a software product that encapsulates one or more software components and its dependencies in a single package, enabling it to run independently and consistently. </del> </span> <span style="color:blue"> A container runtime is a software product that creates the namespace and mount points while unpacking and merging the container image layers. A container image is a software product that encapsulates one or more software components and its dependencies in a single package. For example, an image that complies with the Open Container Initiative's container image specification is a container image. </span> <span style="color:red"> <del> This category includes but is not limited to low-level container runtimes and high-level container runtimes. </del></span></td>
    </tr>
    <tr>
      <td>
